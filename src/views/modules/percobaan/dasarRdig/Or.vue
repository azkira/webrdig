<template>
  <div class="my-2">
    <h1 class="header">Percobaan 1 - Operasi OR</h1>
    <v-row>
      <v-col class="grid">
        <v-row>
          <v-col>
      ...
      <v-col class="grid">
        <v-sheet height="300" width="500" outlined>
          <img
            height="300"
            width="500"
            :src="dataUrl" @error="$event.target.src=require('../../../../assets/default-out.jpg')"
            class="mx-auto"
          />
        </v-sheet>
      </v-col>
  ...
  methods: {
    ...
    get() {
      if (this.temp1 === "0x" && this.temp2 === "0y") {
        this.output = "0";
        this.$mqtt.publish("and-input-active-1", temp1, (err) => {
        if (err) {
          console.log(err);
          //alert("Berhasil Memberi Minum");
        }
      });
      this.$mqtt.publish("and-input-active-2", temp2, (err) => {
        if (err) {
          console.log(err);
          //alert("Berhasil Memberi Minum");
        }
